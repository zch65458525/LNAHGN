...
class data_loader:
    ...
    def load_nodes(self):
        ...
        """
        nodes = {'total':0, 'count':Counter(), 'attr':{}, 'shift':{},'text':{},'text_index':[]}
        ...
        nodes['text_index']=[i for i in range(len(nodes['text'].keys()))]
        index={}
        for i in range(len(nodes['count'])):
            ...
            if shift in nodes['text']:
                
                mat1=[]
                mat=[]
                for j in range(shift, shift+nodes['count'][i]):
                    
                    mat1.append(nodes['text'][j])
                    mat.append(nodes['text_index'][j])
                text[i]=np.array(mat1)
                index[i]=np.array(mat)
            else:
